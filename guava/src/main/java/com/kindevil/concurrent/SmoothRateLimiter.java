...
abstract class SmoothRateLimiter extends RateLimiter {
   ...
   */
    ...
    final long reserveEarliestAvailable(int requiredPermits, long nowMicros) {
        resync(nowMicros);//把nextFreeTicketMicros和now同步
        long returnValue = nextFreeTicketMicros;
        double storedPermitsToSpend = min(requiredPermits, this.storedPermits);//这次能获取的令牌数
        double freshPermits = requiredPermits - storedPermitsToSpend;//需要刷新的令牌数

        long waitMicros = storedPermitsToWaitTime(this.storedPermits, storedPermitsToSpend)
                + (long) (freshPermits * stableIntervalMicros);

        this.nextFreeTicketMicros = nextFreeTicketMicros + waitMicros;//下次获取时间增加
        this.storedPermits -= storedPermitsToSpend;//这次获取的令牌数
        return returnValue;
    }
    ...
    private void resync(long nowMicros) {
        if (nowMicros > nextFreeTicketMicros) {//if nextFreeTicket is in the past, resync to now
