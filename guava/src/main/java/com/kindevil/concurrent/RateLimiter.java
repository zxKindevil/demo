...
public abstract class RateLimiter {
    ...
    private Object mutex() {
        ...
        if (mutex == null) {
            synchronized (this) {//进入同步块的时候可能别的线程已经new了
    ...
    public double acquire(int permits) {
        long microsToWait = reserve(permits);//需要阻塞的时间
    ...
    public boolean tryAcquire(int permits, long timeout, TimeUnit unit) {
        ...
        synchronized (mutex()) {
            ...
            if (!canAcquire(nowMicros)) {
                return false;
            } else {
                microsToWait = reserveAndGetWaitLength(permits, nowMicros);//这里总是0
            }
        }
        stopwatch.sleepMicrosUninterruptibly(microsToWait);//永远不sleep
        return true;
    }

    private boolean canAcquire(long nowMicros) {
     ...
     *
     * @param permits 请求令牌个数
     * @param nowMicros 现在时间
     * @return the required wait time, never negative
     */
    final long reserveAndGetWaitLength(int permits, long nowMicros) {
        long momentAvailable = reserveEarliestAvailable(permits, nowMicros);//直接消耗掉令牌,并且返回需要等待的时间
        return max(momentAvailable - nowMicros, 0);//需要等待的时间
    }
